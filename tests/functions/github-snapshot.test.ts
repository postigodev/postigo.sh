...
import { handleGitHubSnapshot } from '../../src/lib/presence/github-snapshot-handler';
...
describe('/api/github-snapshot', () => {
  it.each(['POST', 'PUT', 'PATCH', 'DELETE'])('rejects %s', async (method) => {
    const response = await handleGitHubSnapshot(new Request('https://portfolio.test/api/github-snapshot', { method }));
    expect(response.status).toBe(405);
  });

  it('rejects every query string', async () => {
    const response = await handleGitHubSnapshot(new Request('https://portfolio.test/api/github-snapshot?user=attacker'));
  ...
  it('returns a bodyless cached HEAD response without calling GitHub', async () => {
    ...
    const response = await handleGitHubSnapshot(new Request('https://portfolio.test/api/github-snapshot', { method: 'HEAD' }));
  ...
  it('never exposes token state or upstream failure text', async () => {
    ...
    const response = await handleGitHubSnapshot(new Request('https://portfolio.test/api/github-snapshot'));
