...
import handler from '../../api/now-playing';
...
describe('/api/now-playing', () => {
  it.each(['POST', 'PUT', 'PATCH', 'DELETE'])('rejects %s without reading credentials', async (method) => {
    const response = await handler.fetch(new Request('https://portfolio.test/api/now-playing', { method }));
    expect(response.status).toBe(405);
    expect(response.headers.get('allow')).toBe('GET, HEAD');
  });

  it('rejects client-selected tokens and accounts', async () => {
    const response = await handler.fetch(new Request('https://portfolio.test/api/now-playing?token=attacker'));
    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: 'INVALID_REQUEST' });
  });

  it('returns a stable unavailable view when owner credentials are absent', async () => {
    const response = await handler.fetch(new Request('https://portfolio.test/api/now-playing'));
  ...
  it('returns headers and no body for HEAD', async () => {
    const response = await handler.fetch(new Request('https://portfolio.test/api/now-playing', { method: 'HEAD' }));
    expect(response.status).toBe(200);
