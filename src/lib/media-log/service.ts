...
export function letterboxdRssUrl(value: string): string | undefined {
  try {
    const url = new URL(value);
    if (
      !["letterboxd.com", "www.letterboxd.com"].includes(
...
async function fetchLetterboxd(
...
): Promise<MediaLogEntry | undefined> {
  ...
  const rssResponse = await fetch(rssUrl, {
    headers: {
      ...
      "user-agent": "postigo.sh/2 media-log",
    },
    signal: signal(),
  });
  if (!rssResponse.ok)
    throw new Error(`Letterboxd returned ${rssResponse.status}`);
  const parsed = new XMLParser({ ignoreAttributes: false }).parse(
    await readTextBounded(rssResponse),
  );
  const rawItems = parsed?.rss?.channel?.item;

  const items = Array.isArray(rawItems) ? rawItems : rawItems ? [rawItems] : [];

  const diaryItems = items
    .filter((item) => {
    ...
    .sort((a, b) => {
      const watchedA = a["letterboxd:watchedDate"];
      const watchedB = b["letterboxd:watchedDate"];

      return watchedB.localeCompare(watchedA);
    });

  const item = diaryItems[0];

  if (!item) return undefined;
  const title =
    typeof item?.["letterboxd:filmTitle"] === "string"
      ? item["letterboxd:filmTitle"].trim()
      : "";
  const watchedAt =
  ...
  const movieResponse = await fetch(
    `https://api.themoviedb.org/3/movie/${tmdbId}?api_key=${encodeURIComponent(tmdbKey)}`,
    {
  ...
  if (!movieResponse.ok)
    throw new Error(`TMDB returned ${movieResponse.status}`);
  const movie = await readJsonBounded<{ poster_path?: unknown }>(movieResponse);
  const posterPath =
    typeof movie.poster_path === "string" &&
    /^\/[a-zA-Z0-9._-]+$/.test(movie.poster_path)
      ? movie.poster_path
      : undefined;
...
async function fetchSteam(
  ...
  const [playerResponse, recentResponse] = await Promise.all([
    fetch(
      `https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?${playerQuery}`,
      { signal: signal() },
    ),
    fetch(
      `https://api.steampowered.com/IPlayerService/GetRecentlyPlayedGames/v1/?${recentQuery}`,
      { signal: signal() },
    ),
  ]);
  if (!playerResponse.ok || !recentResponse.ok)
  ...
  const current =
    Number.isInteger(currentId) && currentId > 0
      ? (games.find((game) => game.appid === currentId) ?? {
          appid: currentId,
  ...
  const game =
    ...
    games.find(
      (candidate) =>
        Number.isInteger(candidate.appid) &&
        candidate.appid! > 0 &&
        candidate.name,
    );
  ...
  return {
    kind: "game",
    activity: current ? "playing" : "played",
    title: game.name.trim(),
    ...(minutes !== undefined && minutes >= 0
      ? { totalHours: Math.round(minutes / 6) / 10 }
      : {}),
...
async function fetchHardcover(
  ...
  const response = await fetch("https://api.hardcover.app/v1/graphql", {
    ...
    body: JSON.stringify({
      query: `query MediaLog { me { user_books(where: {status_id: {_eq: 2}}, order_by: {updated_at: desc}, limit: 1) { user_book_reads { progress_pages } book { title pages image { url } contributions { author { name } } } } } }`,
    }),
  });
  ...
  const body = await readJsonBounded<{
    data?: {
      me?: Array<{
        user_books?: Array<{
          user_book_reads?: Array<{ progress_pages?: unknown }>;
          book?: HardcoverBook;
        }>;
      }>;
    };
  ...
  const progressPages = Math.max(
    0,
    ...(userBook?.user_book_reads ?? []).map(
      (read) => finiteNumber(read.progress_pages) ?? 0,
    ),
  );
  const progressPercent =
    pages && pages > 0
      ? Math.min(100, Math.round((progressPages / pages) * 100))
      : undefined;
  return {
    kind: "book",
    activity: "reading",
...
export async function getMediaLog(
  runtimeEnvironment: Environment = process.env,
): Promise<MediaLogResponse> {
  const settled = await Promise.allSettled([
    fetchLetterboxd(runtimeEnvironment),
    fetchSteam(runtimeEnvironment),
    fetchHardcover(runtimeEnvironment),
  ]);
  return {
    entries: settled.flatMap((result) =>
